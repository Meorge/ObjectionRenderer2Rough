from ace_attorney_scene import (
    SPR_PHX_NORMAL_T,
    SPR_PHX_NORMAL_I,
    SPR_PHX_SWEAT_T,
    SPR_PHX_SWEAT_I,
...
test_dialogue_1 = f"<music start cross-moderato/><nametag \"Phoenix right\"/><showbox/>" + \
...
test_dialogue_2 = f"<nametag \"Phoenix right\"/><showbox/>{SPR_PHX_SWEAT_T}...Hello?" + \
    f"{SPR_PHX_SWEAT_I}<wait 0.25/>{SPR_PHX_SWEAT_T} ...Is anyone there?" + \
    f" {SPR_PHX_SWEAT_I}{END_BOX}<hidebox/>"

test_dialogue_3 = f"<music stop/>{OBJ_EDW}<pan right/><wait 1/>" + \
